...
オフラインで動作する学習カードアプリ。
IndexedDB + Dexie.js によるローカル保存、PWA によるホーム画面追加に対応。

## 目的（Why）

医療・ビジネス現場で
**「すぐに使える・ネット不要・軽量な学習ツール」** を提供するために開発。
...
- **Dexie.js による高速クエリ**
→ 大量カードでもストレスなく検索・操作可能
- **UI/UX の最適化**
→ 片手操作・カードスワイプ・視認性を重視
- **Clean Architecture を意識した構成**
→ UI とデータ層を分離し、拡張性を確保
- **PWA 対応**
→ ホーム画面追加、スプラッシュ画面、オフラインキャッシュ
